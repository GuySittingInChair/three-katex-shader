export default {
  name: 'Audio Pulse',
  description: 'Bass/mid/treble-reactive rings. Click "Enable Audio" to feed it real sound.',
  tags: ['audio', 'reactive'],
  category: 'Audio-Reactive',
  mode: 'shader',
  shaderLang: 'glsl',
  latex: 'r(\\theta,t) = r_0 + b(t)\\sin(3\\theta) + m(t)\\sin(7\\theta) + h(t)\\sin(13\\theta)',
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      float wobble = uBass * sin(a * 3.0 + uTime * uSpeed)
                   + uMid * sin(a * 7.0 - uTime * uSpeed * 1.3)
                   + uTreble * sin(a * 13.0 + uTime * uSpeed * 0.7);
      wobble *= uSensitivity;
  ...
  update(ctx, state) {
    ...
    if (ctx.audio?.enabled) {
      ...
      // Idle fallback so it still looks alive before audio is enabled.
